...
std::vector<int> getMinDistances(const std::vector<std::unordered_map<std::string, bool>>& blocks, const std::string& req) {
    ...
    for (int i = 0; i < blocks.size(); ++i) {
        ...
        if (it != blocks[i].end() && it->second)
            closestIndexForReq = i;
        if (closestIndexForReq != INF)
            minDistancesPerRequired[i] = i - closestIndexForReq;
    }
    ...
    for (int i = blocks.size() - 1; i >= 0; --i) {
        ...
        if (it != blocks[i].end() && it->second)
            closestIndexForReq = i;
        if (closestIndexForReq != INF)
            minDistancesPerRequired[i] = std::min(minDistancesPerRequired[i], closestIndexForReq - i);
    }
