...
i64 maxAbsoluteDiffBF(const vector<int>& nums) {  // f(i,j)=|A[i]-A[j]|+|i-j| the smallest possible is 0
    int n = nums.size();
    i64 best = 0;  //
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < i; ++j) {
...
i64 maxAbsoluteDiff(const vector<int>& A) {  // f(i,j)=|A[i]-A[j]|+|i-j| the smallest possible is 0
    int n = A.size();
    int a0 = A[0];
    int i0 = 0;
